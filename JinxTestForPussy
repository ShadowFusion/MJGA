...
function Jinx:killsteal()
    ...
    local definedDistance = self.shadowMenu.killsteal.killstealrrange:Value()
        if Ready(_R) and target and IsValid(target) and (target.health <= rdmg) and self.shadowMenu.killsteal.killstealr:Value() and GetDistanceSqr(myHero.pos, target.pos) <=  definedDistance then
            self:CastR(target)
        end
