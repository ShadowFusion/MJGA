...
do
    ...
    local Files = {
        Lua = {
            Path = SCRIPT_PATH,
            Name = "Soraka.lua",
            Url = "https://raw.githubusercontent.com/ShadowFusion/MJGA/master/Soraka.lua"
        },
        Version = {
            Path = SCRIPT_PATH,
            Name = "Soraka.version",
            Url = "https://raw.githubusercontent.com/ShadowFusion/MJGA/master/Soraka.version"    -- check if Raw Adress correct pls.. after you have create the version file on Github
        }
    }
...
local Icons = {
    ["SorakaIcon"] = "https://vignette.wikia.nocookie.net/leagueoflegends/images/6/65/Soraka_OriginalSquare.png",
...
function Soraka:LoadMenu()
    self.shadowMenu = MenuElement({type = MENU, id = "shadowSoraka", name = "Shadow Soraka", leftIcon = Icons["SorakaIcon"]})
...
function Soraka:junglekillsteal()
...
function Soraka:HasSecondQ()
    return Soraka:GotBuff(myHero, "SorakaQ") > 0
end

function Soraka:GotBuff(unit, buffname)
    for i = 0, unit.buffCount do
        local buff = unit:GetBuff(i)
