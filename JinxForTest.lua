...
function Jinx:__init()
    ...
    self.R = {Type = _G.SPELLTYPE_LINE, Range = 100000, Radius = 225, Speed = 1500}
...
function Jinx:LoadMenu()
    ...
    self.shadowMenu.killsteal:MenuElement({id = "killstealrrange", name = "Range to use Killsteal R", value = self.W.Range, min = self.W.Range, max = 5000})
...
function Jinx:killsteal()
    ...
    local definedDistance = self.shadowMenu.killsteal.killstealrrange:Value()
    local distanceBetween =  GetDistanceSqr(myHero.pos, target.pos)
        if Ready(_R) and target and IsValid(target) and (target.health <= rdmg) and self.shadowMenu.killsteal.killstealr:Value() and distanceBetween <= definedDistance then
            self:CastR(target)
        end
